...
/**
 * สมองกลน้องนำทาง - เวอร์ชั่น Ultra Stable (Kiosk Professional - All-in-One)
 * แก้ไขมิติ: ทักซ้ำ, หน้าจอค้าง, Safety Timeout, และระบบสัมผัส
 */
...
const IDLE_TIME_LIMIT = 45000; // ตั้งไว้ 45 วินาทีเพื่อความสมดุล
let video = document.getElementById('video');
let cocoModel = null; 
let isDetecting = true; 
let personInFrameTime = null; 
...
/**
 * 1. ระบบจัดการสถานะและความเสถียร (State & Safety)
 */

// ปลดล็อคสถานะแบบรวมศูนย์
...
// อัปเดตเวลาการใช้งานล่าสุด (ใช้ทั้งกล้องและการสัมผัส)
function updateInteractionTime() {
    lastSeenTime = Date.now();
    restartIdleTimer();
}

// ลงทะเบียนการตรวจจับการสัมผัสทั่วหน้าจอ
...
/**
 * 2. ระบบ Reset หน้าจอ (Hard Reset with Touch Support)
 */
function resetToHome() {
    const now = Date.now();
    // เช็คว่าไม่มีการเคลื่อนไหวทั้งจากกล้องและจากการสัมผัสหน้าจอ
...
/**
 * 3. ระบบดวงตา AI (แก้ปัญหาทักซ้ำ - Detection Logic)
 */
async function initCamera() {
    try {
        const stream = await navigator.mediaDevices.getUserMedia({ 
...
async function detectPerson() {
    ...
    if (person) {
        lastSeenTime = Date.now(); // อัปเดตเวลาที่พบคนตลอดเวลาแม้ Busy
        restartIdleTimer();

        if (personInFrameTime === null) personInFrameTime = Date.now();

        // เงื่อนไขทักทาย: ไม่ยุ่ง AND ยังไม่ทัก AND ยืนแช่ 1.5 วิ
        if (!window.isBusy && !window.hasGreeted) {
            if (Date.now() - personInFrameTime >= 1500) {
                greetUser();
            }
        ...
        // ต้องไม่เจอคนติดต่อกัน 7 วินาที ถึงจะถือว่าคนเดินออกไปแล้วจริงๆ
...
async function getResponse(userQuery) {
    ...
    // Safety: ป้องกันกรณีเน็ตค้างขณะ Fetch ข้อมูล
...
function speak(text) {
    ...
    // Safety Unlock: ป้องกันค้างโดยคำนวณจากความยาวประโยค
    const safetyTime = (text.length * 200) + 5000;
    
    clearTimeout(speechSafetyTimeout);
    speechSafetyTimeout = setTimeout(() => {
        console.warn("Safety Unlock: Speech Engine stuck, forcing idle.");
        window.isBusy = false;
        updateLottie('idle');
    }, safetyTime);
...
/**
 * 6. ระบบสนับสนุนอื่นๆ (UI & Algorithm)
 */
async function initDatabase() {
    try {
        const res = await fetch(GAS_URL, { redirect: 'follow' });
